...
- `sql/queries/chirps.sql` has `CreateChirp` (INSERT with body and user_id params), `GetChirps`, `GetChirpsByAuthor` (filters by `user_id`, both ordered by `created_at ASC`), `GetChirp` (by id), and `DeleteChirp` (by id) queries.
