...
- `POST /api/chirps`, `GET /api/chirps`, `GET /api/chirps/{chirpID}`, `DELETE /api/chirps/{chirpID}`: chirp CRUD; create requires a valid `Authorization: Bearer <JWT>` (the user id comes from the token, not the body), validates body length (140 chars max), cleans profanity, and saves via SQLC `CreateChirp`. `GET /api/chirps` accepts an optional `?author_id=<uuid>` query param — when present, filters at the DB level via SQLC `GetChirpsByAuthor` (`400` if not a valid UUID); otherwise returns all chirps via `GetChirps`. Both DB queries order by `created_at ASC`; an optional `?sort=asc|desc` query param (default `asc`, `400` for any other non-empty value) re-sorts the fetched chirps in memory via `sort.Slice` — `desc` reverses, `asc`/omitted keeps the DB's ascending order. Delete requires a valid access token, then does a single conditional delete via SQLC `DeleteChirpByOwner` (`WHERE id=$1 AND user_id=$2 RETURNING id`) — returns `404` for both "doesn't exist" and "exists but isn't yours" (no `403`, by design — see Key Design Decisions), `204` on success
- `POST /api/polka/webhooks`: webhook from the "Polka" payment provider, authenticated via `Authorization: ApiKey <key>` checked against `apiConfig.polkaKey` using `crypto/subtle.ConstantTimeCompare` (`401` if missing/wrong — checked before parsing the body), for the same timing-safety reason password hashes are compared via argon2id rather than `==`. Body is `{"event": "...", "data": {"user_id": "..."}}`; any event other than `user.upgraded` is ignored with an immediate `204`. For `user.upgraded`, upgrades the user via SQLC `UpgradeUserToChirpyRed`, returning `404` if the user id doesn't exist or `204` on success
...
- `sql/queries/chirps.sql` has `CreateChirp` (INSERT with body and user_id params), `GetChirps`, `GetChirpsByAuthor` (filters by `user_id`, both ordered by `created_at ASC`), `GetChirp` (by id), and `DeleteChirpByOwner` (`DELETE ... WHERE id=$1 AND user_id=$2 RETURNING id` — one round trip that both authorizes and deletes) queries.
...
- API additions:
   ...
   - `PUT /api/users` requires a valid access token (`requireAccessToken`); the JWT's user id (not the body) determines which row is updated. Takes the same `{ "email": "...", "password": "..." }` body, re-hashes the password, and updates that user's row via SQLC `UpdateUser`. Returns `200` with the updated `userResponse` (no password), `401` for a missing/invalid token, `400` for a missing email/password, `404` if `UpdateUser` returns `sql.ErrNoRows` (the token's user no longer exists, e.g. deleted between issuance and use).
...
- **TestGetChirpsEndpoint**: also covers `?author_id=` filtering (asserts the SQL sent has a `WHERE user_id = $1` clause, i.e. filtering happens in the DB not in memory), a `400` for a non-UUID `author_id`, `?sort=desc` reversing the (already-ascending) DB result, and a `400` for an invalid `?sort=` value (e.g. `dsc`)
- **TestDeleteChirpEndpoint**: subtests against `DELETE /api/chirps/{chirpID}` covering owner deletion (204), non-owner attempt (404, not 403 — see Key Design Decisions), missing chirp (404), and missing Authorization header (401)
...
- **TestUpdateUserEndpoint**: table of subtests covering `PUT /api/users` with a valid access token (200, updated email returned, no password field), the token's user no longer existing (404), missing Authorization header (401), and malformed/invalid JWT (401)
...
- **Auth Helpers**: `apiConfig.requireBearerToken` (extract `Authorization: Bearer <token>` or write 401) and `apiConfig.requireAccessToken` (also validates it as a JWT) centralize the bearer-token check used by `handlerChirpsCreate`, `handlerRefresh`, and `handlerRevoke` instead of repeating it per handler. `internal/auth.GetBearerToken`/`GetAPIKey` both delegate to a private `extractAuthHeader(headers, prefix)` so the two schemes (`Bearer `/`ApiKey `) share one implementation
- **DB-Required Guard**: `apiConfig.requireDB(w) bool` centralizes the "no database configured → 500" check used by every handler except `handlerReset` (which deliberately still resets `fileserverHits` even with no DB configured, per `TestResetEndpoint`)
- **Chirp Lookup Helper**: `apiConfig.lookupChirpOrRespond(w, r) (database.Chirp, bool)` centralizes parsing `{chirpID}` and fetching it (404/500 on failure), shared by `handlerChirpsGetByID` and `handlerChirpsDelete`
- **Shared Credentials Decoding**: `decodeCredentialsOrRespond(w, r) (credentialsParams, bool)` centralizes the `{email, password}` JSON decode + required-field check shared by `handlerUsersCreate`/`handlerUsersUpdate`. `handlerLogin` keeps its own inline copy deliberately — it returns different status codes (`401` instead of `500`/`400`) on the same failures, since revealing *why* a login request was malformed would leak more than a generic "incorrect email or password"
...
- **Chirp Delete Returns 404 for Non-Owners, Not 403**: `handlerChirpsDelete` does a single conditional `DeleteChirpByOwner` (`WHERE id=$1 AND user_id=$2`) instead of a `GetChirp` + ownership check + `DeleteChirp` (2 round trips). Deliberate tradeoff, decided 2026-06-26: one DB round trip per delete instead of two, at the cost of merging "chirp doesn't exist" and "chirp exists but isn't yours" into the same `404` — a non-owner can no longer distinguish the two from the response, which is the same probing-resistance property `POST /api/revoke` already relies on (see Idempotent Revoke above)
