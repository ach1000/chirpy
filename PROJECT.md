...
- **TestGetChirpsEndpoint**: also covers `?author_id=` filtering (asserts the SQL sent has a `WHERE user_id = $1` clause, i.e. filtering happens in the DB not in memory) and a `400` for a non-UUID `author_id`
